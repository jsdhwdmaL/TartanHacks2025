...
class Player:
    ...
    def move(self, keys, width, height):
        ...
        if ((keys[pygame.K_RIGHT] or keys[pygame.K_d]) and (self.rect.x <= (width - playerwidth))):
            self.rect.x += self.speed
        ...
        if ((keys[pygame.K_DOWN] or keys[pygame.K_s]) and (self.rect.y <= (height - playerheight))):
            self.rect.y += self.speed
            self.image = pygame.image.load("assets/mermaid_forward.png")
            self.image = pygame.transform.scale(self.image, (playerheight, playerwidth))
